...
def generate_study_guide_full_context(knowledge_packet: str) -> str:
    ...
    template = """
    You are a professional learning assistant trained to help students revise technical lectures effectively.
    Your job is to generate a factually correct, well-structured, and highly useful study guide based ONLY on the provided lecture context.

    🔒 STRICT RULES:
    - Do NOT invent or add anything that is not explicitly present in the input context.
    - Do NOT use prior knowledge or assumptions — only use what is grounded in the full context.
    - Preserve technical accuracy and include relevant code/config examples from the content.

    Provided Full Lecture Context:
    ----------------
    {full_context}
    ----------------
    ...
    A quick, high-level overview of the lecture's key takeaways.

    2. 📝 **Improved Summary (Markdown)**  
    A full, improved markdown summary covering major topics and key flow.

    3. 📌 **Revision Notes**  
    A bulleted list of all core definitions, bullet points, processes, and logical structures from the lecture.

    4. 🧠 **Important Concepts**  
    Highlight and define critical concepts, frameworks, terminology, and tools introduced.

    5. ❓ **Interview-style Questions & Answers**  
    Include as many as conceptual or technical Q&As that an interviewer or instructor might ask.

    6. 🛠️ **Practical Setup & Code Snippets** (Optional)  
    If present in the content, extract any command-line setup, config files, Python SDK usage, or tool-building code snippets — and explain them clearly.
    If nothing is available, return “None.”

    7. ⚠️ **Misunderstood or Confusing Topics** (Optional)  
    Identify any topics that could be confusing for learners and explain them. If none, say “None.”

    Ensure all output is in clean markdown format, using section headings and subpoints as appropriate.
    """
    
    prompt = ChatPromptTemplate.from_template(template)
    llm = OllamaLLM(model="deepseek-coder-v2:lite", temperature=0.1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output_dir", default="output_short_notes", help="Output directory for .md files")
...
# <lecture-name>Python Functions</lecture-name>
# <transcript>
# Today we covered Python functions, including how to define them using `def`, pass parameters, return values, and the use of default arguments and *args/**kwargs...
# </transcript>
# <imp-concept>def keyword, return statement, default arguments, *args, **kwargs</imp-concept>
# <notes>Functions are reusable code blocks. Python uses 'def' to define them.</notes>
